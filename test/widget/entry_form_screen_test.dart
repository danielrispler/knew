...
class FakeWordsRepository implements WordsRepository {
  ...
  Future<bool> applyEnrichment(Entry original, List<Meaning> meanings) async {
    entries[original.id] = original.copyWith(meanings: meanings);
    return true;
  }
...
void main() {
  group('EntryFormScreen Instant AI Card & Collapsible Drawer Tests', () {
    testWidgets(
      'displays Hero Term Field and collapsed custom drawer by default for new entries',
      (WidgetTester tester) async {
        ...
        expect(
          find.text('Enter English term or Hebrew word...'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'triggers automatic lookup with 600ms debounce when 2+ characters typed',
      (WidgetTester tester) async {
        final fakeRepo = FakeWordsRepository();
        ...
        await tester.enterText(
          find.byKey(const Key('hero_term_field')),
          'persistent',
        );
        ...
        expect(
          find.text('Continuing firmly in a course of action.'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'triggers immediate lookup on keyboard submit action without waiting for debounce',
      (WidgetTester tester) async {
        final fakeRepo = FakeWordsRepository();
        final mockHttpClient = MockClient((request) async {
          final payload = {
    ...
    testWidgets('clear button clears input and resets AI card', (
      WidgetTester tester,
    ) async {
      final fakeRepo = FakeWordsRepository();
      final mockHttpClient = MockClient((request) async {
      ...
      expect(find.byKey(const Key('instant_ai_card')), findsNothing);
      final heroField = tester.widget<TextField>(
        find.byKey(const Key('hero_term_field')),
      );
    ...
    testWidgets(
      'hebrew translation chips are selectable (RTL) with primary pre-selected and toggling updates selection',
      (WidgetTester tester) async {
        final fakeRepo = FakeWordsRepository();
        final mockHttpClient = MockClient((request) async {
          final payload = {
    ...
    testWidgets(
      'multi-sense switcher allows switching between senses when Gemini returns >1 meaning sense',
      (WidgetTester tester) async {
        final fakeRepo = FakeWordsRepository();
        final mockHttpClient = MockClient((request) async {
          final payload = {
    ...
    testWidgets(
      'spelling suggestion chip displays on invalid input and one-tap corrects and looks up',
      (WidgetTester tester) async {
        final fakeRepo = FakeWordsRepository();
        final mockHttpClient = MockClient((request) async {
        ...
        expect(
          find.byKey(const Key('spelling_suggestion_chip')),
          findsOneWidget,
        );
    ...
    testWidgets(
      'shows error and auto-expands custom drawer on lookup failure (no API key)',
      (WidgetTester tester) async {
        final fakeRepo = FakeWordsRepository();
        const testSettings = SettingsState(
          sessionSize: 20,
        ...
        // Error message shown
        expect(
          find.textContaining('Add your Gemini API key in Settings'),
          findsOneWidget,
        );
        ...
        expect(
          find.byKey(const Key('manual_meanings_section')),
          findsOneWidget,
        );
      },
    );
    ...
    testWidgets(
      'collapsible custom drawer auto-expands when editing existing entry with custom metadata',
      (WidgetTester tester) async {
        final fakeRepo = FakeWordsRepository();
        ...
        expect(
          find.text('We must persevere despite obstacles.'),
          findsOneWidget,
        );
      },
    );
    ...
    testWidgets('pronunciation speaker button triggers TTS', (
      WidgetTester tester,
    ) async {
      final fakeRepo = FakeWordsRepository();
