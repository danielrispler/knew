...
void main() {
  group('GeminiModels', () {
    test('getFallbackSequence returns 2-hop sequence [primary, 3.5-flash-lite] for non-lite models', () {
      expect(
    ...
    test('getFallbackSequence returns single hop [3.5-flash-lite] when primary model is flash-lite', () {
      expect(
  ...
  group('GeminiClient', () {
    ...
    test('sets maxOutputTokens to 512 and includes zero thinking budget for primary model', () async {
      late http.Request capturedRequest;
      final mockHttpClient = MockClient((request) async {
        capturedRequest = request;
        final responsePayload = {
    ...
    test('retries without thinkingConfig when primary model returns 400 for unsupported thinkingConfig', () async {
      int attempt = 0;
      ...
      expect(firstBody['generationConfig']['thinkingConfig'], equals({'thinkingBudget': 0}));
      final secondBody = jsonDecode(capturedRequests[1].body);
      expect(secondBody['generationConfig'].containsKey('thinkingConfig'), isFalse);
    });
    ...
    test('logs timing per model attempt in structured format', () async {
      ...
      final logRegex = RegExp(r'^\[GeminiClient\] "apple" -> model: gemini-3.8-flash \| duration: \d+ms \| status: 200$');
      expect(logRegex.hasMatch(logs[0]), isTrue, reason: 'Expected log to match pattern but got: ${logs[0]}');
    });
    ...
    test('lookupWithFallback falls back directly to 3.5-flash-lite when 3.8 returns 429 quota exhausted', () async {
      ...
      expect(result, isA<GeminiSuccessResult>());
      expect(requestedModels, equals(['gemini-3.8-flash', 'gemini-3.5-flash-lite']));
    });

    test('lookupWithFallback executes only a single hop when primary model is 3.5-flash-lite', () async {
      final requestedModels = <String>[];
      final mockHttpClient = MockClient((request) async {
        final uriPath = request.url.path;
    ...
    test('lookupWithFallback attempts custom model first then falls back directly to 3.5-flash-lite', () async {
      final requestedModels = <String>[];
      final mockHttpClient = MockClient((request) async {
        final uriPath = request.url.path;
      ...
      expect(result, isA<GeminiSuccessResult>());
      expect(requestedModels, equals(['custom-experimental-model', 'gemini-3.5-flash-lite']));
    });

    test('lookupWithFallback falls back to 3.5-flash-lite on 503 server error', () async {
      final requestedModels = <String>[];
      final mockHttpClient = MockClient((request) async {
        final uriPath = request.url.path;
        if (uriPath.contains('gemini-3.8-flash')) {
    ...
    test('logs timing on error and timeout', () async {
      ...
      await expectLater(
        () => client.lookup(input: 'hello', apiKey: testApiKey, model: 'timeout-model'),
        throwsA(isA<GeminiException>().having((e) => e.errorType, 'errorType', equals(GeminiErrorType.timeout))),
      );
      expect(logs.any((l) => l.contains('status: timeout')), isTrue);

      await expectLater(
        () => client.lookup(input: 'world', apiKey: testApiKey, model: GeminiModels.gemini38Flash),
        throwsA(isA<GeminiException>().having((e) => e.errorType, 'errorType', equals(GeminiErrorType.quotaExhausted))),
      );
