...
void main() {
  ...
  group('PracticeScheduler - Incorrect Grade Level & Interval Transitions', () {
    ...
    test('Level 4 -> Level 3 (due tomorrow)', () {
      final entry = createTestEntry(id: '1', level: 4, dueDate: '2026-09-15', timesWrong: 1);
      final updated = PracticeScheduler.gradeIncorrect(entry, now: now);
  ...
  group('PracticeScheduler - DST and Calendar Month Math', () {
    test('Calculates local calendar dates correctly across month boundaries and leap year', () {
    ...
    test('Calculates local calendar dates across DST spring forward', () {
      ...
      final entry = createTestEntry(id: '1', level: 3, dueDate: '2026-03-28'); // level 3 -> 4 (7 days)
  ...
  group('PracticeScheduler - Grade Override (Count as Correct)', () {
    ...
    test('Replaces wrong grade with correct grade from original pre-answer snapshot', () {
      ...
      final overridden = PracticeScheduler.overrideWrongWithCorrect(original, now: now);
      ...
      expect(overridden.timesWrong, equals(1)); // wrong count restored from original, not incremented
      expect(overridden.lastReviewedAt, equals(now.toUtc().toIso8601String()));
    });
  });
}
